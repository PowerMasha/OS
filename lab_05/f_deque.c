...
_Bool empty(f_deque* deque){ // если дек пустой - 1, иначе 0
    if(deque->size == 0)
        return 1;
    else
        return 0;
...
void push_back(f_deque* deque, double flt){  // вставка элемента в конец дека (справа)
...
void pop_back(f_deque* deque){ // удаление элемента справа
    if (empty(deque)){
...
void push_front(f_deque* deque, double flt){ // добавление элемента слева
    node* newnode = (node*)malloc(sizeof(node));
    newnode->prev = NULL;
    newnode->data = flt;
    if(empty(deque)){
...
void pop_front(f_deque* deque){ // удаление элемента слева
    if (empty(deque)){
...
void print_node(node* node){ // вывод элемента
    printf("%f", node->data);
    if(node->next == NULL){
        return;
    }
...
void print(f_deque* deque){ // вывод дека
    if (empty(deque)){
...
void destroy(f_deque* deque) { //удаление всего дека
    while(!empty(deque)){
        pop_back(deque);
    }
    free(deque);
